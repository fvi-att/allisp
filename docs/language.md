...
(re-fix <form> :rounds 16 :model :opus) ; 値の中の intermediate-code も再帰的に fix する
...
**`re-fix`** は、値の中に含まれる `intermediate-code` も再帰的に探し、それぞれへ Fix モードを繰り返し適用する。たとえば `probe-spec` の `:findings` のように intermediate-code がリストの内側にある場合も、`(re-fix (probe-spec name))` の一段で処理できる。各ノードは実行可能になると上記と同じ `fixed` 監査レコードに置き換わり、その `:value` にさらに intermediate-code があれば続けて処理される。監査用の `:source` と `:code` は再処理しない。停止しない lowering を防ぐため、各ノードの上限は `:rounds`（既定 16）であり、上限まで未解決なら最後の intermediate-code をその位置に残す。
...
result ファイルの各フォーム `(result :v 3 :n K :form F :value V)` は組み込みの `result` フォームとして評価され、`:form` を再評価せず、`:value` を未評価のデータとして扱う。v3 は example 追加と、hash/evaluator/prompt/model が完全一致する check/probe 証明も復元する。v1/v2 は読み取り可能だが、新しい証明は復元しない。
...
## 仕様の第一級構文（defspec / example / check-spec / derive / probe-spec / verify）
...
(example slugify :name :hello-world
  ...
  :covers (:only-chars :no-edge-hyphen))
```
...
- スキーマ検査は決定論的（オラクルなし）。トップ条項の重複は `:spec-duplicate-clause`、
  不変条件名の重複は `:spec-duplicate-invariant`。inline `:examples` は
  `:spec-inline-examples` になり、top-level `example` だけが利用できる。
...
- アクセサ（決定論的ビルトイン）: `(spec-invariants s)` → 条項名リスト、
  ...
  `(spec-example s :hello-world)` → 名前検索、
  `(spec-examples-for-input s input)` → 同一入力の全実例（追加順）。

### example — 自然言語 context を持つ名前付き実例

```lisp
(example <defspec名>
  :name <keyword> :in <未評価データ> :out <未評価データ>
  :context <空でない文字列>
  [:covers (<invariant名> ...)]
  [:depends-on (<先行top-level定義名> ...)])
```

`example` は同一ファイル内で先に現れた `defspec` にだけ追加できる top-level
構文である。全フィールドは未評価データとして保存され、`:name` だけが同一 spec
内の identity になる。同じ `:in`、同じ内容、異なる `:out` も名前が違えば登録でき、
妥当性は context を含む check/probe が判定する。context は補足説明ではなく、
背景・条件分岐を含む規範的要件である（1件4000文字、spec合計16000文字）。

`:depends-on` は先行する同一ファイルの `defun` / `define` / `defmacro` / `def` /
`defvar` / `defparameter` / `defspec` を明示する。依存は再帰追跡され、正規化した
定義内容が hash と監査 prompt に入る。plugin binding と defspec 循環は認めない。

### check-spec — 不変条件×実例の決定論的検査

```lisp
(check-spec slugify [:model M] [:fresh F])
;; 全緑: (spec-check :spec slugify
;;          :invariant-checked 8 :context-checked 3
;;          :violations () :skipped (...) ...)
;; 違反: (error :type :spec-violation :detail (spec-check ... :violations ((:invariant :no-edge-hyphen :example (:in "!!!" :out "-")))))
```

- 各不変条件を **1 条項 = 1 オラクル呼び出し**で `(lambda (in out) ...)` へ lowering
  する。さらに signature + context + `:covers` の条項だけから context 述語を作る。
  具体的な in/out は lowering prompt に入れず、同一 context は predicate cache を共有する。
- 違反・skip の各 entry は `:kind :invariant|:context` を持つ。不一致は
  `:spec-violation`、lowering不能は `:skipped`。実例が0件なら `:spec-no-examples`。
...
- `:from` は仕様名（またはリスト）。束縛されていなければ `:derive-unknown-spec` のエラー値になり、
  オラクルは呼ばれない。
- 現在 hash に一致する `check-spec` と、focusなしの完全な `probe-spec`
  (`:complete t`, findings 0)が必須。`:skipped` は通常停止し、CLI
  `--ignore-skip` でのみ許可できる。違反・probe finding・監査未完了は回避できない。
  依存 `defspec` も同じ証明を再帰的に要求する。
...
- `(stale :target ... :from <spec>)` — spec/example/依存定義が変わった。ソースを再実行すれば解消
- `(drifted :target ...)` — 生成物が手編集された。直したい内容は仕様側の条項に書くこと
- `(missing :target ...)` / `(unknown :target ...)` — 生成物が消えた / 記録ソースに defspec が見つからない
- `(invalid :target ... :spec ... :error ...)` — malformed example、重複名、未知のcovers/依存など
...
;; => (spec-findings :spec slugify :complete t :count 1
;;      :findings ((intermediate-code
;;                   :reason (:why ":collapse demands one hyphen while :no-edge-hyphen forbids it"
;;                            :how "amend :collapse to discard edge runs, then add (:in \"!!!\" :out \"\")") ...)))
```

- query-spec 慣習（受動・単一質問）の一般化。オラクルが invariant/context 間の矛盾、
  未規定領域、観測不能な条件、正当化不能な依存を能動的に探索し、
  各穴を intermediate-code（`:why` = 矛盾条項、
  `:how` = 決着させる実例または invariant 改訂）として返す。
- 生成物は**実行されない**。監査完了は `:complete t`、穴がなければ `:findings ()`。
  判定不能は `:complete nil` であり、findings 0でもderiveを許可しない。
  応答が契約に合わなければ `:invalid-probe-response` のエラー値。
- キャッシュ粒度は**意図的に仕様全体**: 穴は条項の組に宿るため、どの条項を編集しても再探索される
  （check-spec の条項単位キャッシュとの対比）。`:focus` で監査対象の条項を絞れるが、
  部分監査はderiveの証明にはならない。
...
- **特殊形式**：`quote`、`quasiquote`、`if`、`cond`、`when`、`unless`、`let`、`let*`、`lambda`、`progn`、`and`、`or`、`defun`、`define`、`defmacro`、`def`、`defvar`、`defparameter`、`setq`、`setf`、`push`、`incf`、`decf`、`@use`、`llm`、`pure`、`fix`、`re-fix`、`defer`、`deprecate`、`result`、`intermediate-code`、`markdown->lisp`、`probe-spec`、`verify`
...
- **組み込みマクロ**：`generate-file`、`goal`、`constraint`、`solve`、`defspec`、`example`、`check-spec`、`derive`
- **仕様アクセサ組み込み**：`spec-invariants`、`spec-invariant`、`spec-examples`、`spec-example`、`spec-examples-for-input`
- **予約語**：`result`、`fix`、`re-fix`、`markdown->lisp`、`defspec`、`example`、`check-spec`、`derive`、`probe-spec`、`verify`（ユーザー DSL の演算子名には使えない）
