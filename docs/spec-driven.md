...
条項名の重複、`(:name "1文")` 形でない不変条件、`:in`/`:out` を欠く実例は
...
- **実例は境界を選ぶ**。実例はそのままテストの assertion になり、
  `check-spec` の検査対象になる。解釈が割れそうな入力ほど価値が高い。
...
各不変条件が 1 回のオラクル呼び出しで `(lambda (in out) ...)` 述語に lowering され、
全実例へ決定論的に適用される。`(:in "!!!" :out "")` のような実例を足した瞬間に、
`:collapse`(全体が 1 本のハイフンになる)と `:no-edge-hyphen`(端のハイフン禁止)の
矛盾が「どの条項がどの実例で偽か」として名指しされる。

- 述語のキャッシュは**条項単位**である。1 条項を編集しても、他の条項の述語は
  キャッシュから再生される。実例の追加・編集は LLM 呼び出しゼロで再検査される。
